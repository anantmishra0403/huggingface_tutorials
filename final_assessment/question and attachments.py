...
class BasicAgent:
    def __init__(self,):
        ...
        print("BasicAgent initialized with ReActAgent.")

    def __call__(self, question: str) -> str:
        print(f"Agent received question (first 50 chars): {question[:50]}...")

        try:
            # run the query synchronously using asyncio
            result = asyncio.run(self._run_agent(question))
    ...
    async def _run_agent(self, question: str) -> str:
        ...
        handler = self.agent.run(question, ctx=ctx)

        final_answer = []
        async for ev in handler.stream_events():
            if isinstance(ev, AgentStream):
                final_answer.append(ev.delta)
